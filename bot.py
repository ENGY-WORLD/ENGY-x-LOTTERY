...
def stats_text() -> str:
    ...
    lines = [
        ...
        f"📈 TOTAL BET: {s['total']}",
        f"🎯 WIN ACCURACY: *{predictor.acc:.1f}%*",
    ]
    ...
    lines.append("\n_EXPECTED: 70-80% ON A TRUE RNG_")
    return "\n".join(lines)
...
def req_approved(fn):
    async def w(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not is_approved(uid):
            doc = users.find_one({"user_id": uid})
            msg = ("LICENSE EXPIRED ,CONTACT OWNER TO RENEW."
                   if doc else "LICENSE NOT ACTIVATED. CONTACT OWNER.")
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in ADMIN_IDS:
        exp_line = "OWNER —AUTO LICENSE ACTIVATED"
    elif is_approved(uid):
        doc = users.find_one({"user_id": uid})
        exp_line = f"LICENSE ACTIVATED — {days_left(doc['expires_at'])} remaining"
    else:
        exp_line = "LICENSE NOT ACTIVATED — CONTACT OWNER"
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if uid in ADMIN_IDS:
        await update.message.reply_text("OWNER : AUTO LICENSE ACTIVATED")
        return
    doc = users.find_one({"user_id": uid})
    if not doc:
    ...
    await update.message.reply_text(
        f"✅ *LICENSE ACTIVATED*\n\n"
        f"USER ID: `{uid}`\n"
        f"EXPIRE: `{exp.strftime('%Y-%m-%d %H:%M UTC')}`\n"
